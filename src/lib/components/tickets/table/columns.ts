...
import DataTableCheckbox from './ticket-table-checkbox.svelte';
import DataTableActions from './ticket-table-actions.svelte';
...
export const columns: ColumnDef<TicketList>[] = [
	...
	},
	{
		...
		cell: ({ row }) => {
			const statusCellSnippet = createRawSnippet(() => ({
				render: () =>
					`<div class="flex items-center" style="color: ${row.original.status.color};"><div style="background-color: ${darkenColor(row.original.status.color, 20)};" class="w-1.5 h-1.5 rounded-full mr-1"></div><span>${row.original.status.name}</span></div>`
			}));
	...
	},
	{
		...
		cell: ({ row }) => {
			const statusCellSnippet = createRawSnippet(() => ({
				render: () =>
					`<div class="flex items-center" style="color: ${row.original.priority.color};"><div style="background-color: ${darkenColor(row.original.priority.color, 20)};" class="w-1.5 h-1.5 rounded-full mr-1"></div><span>${row.original.priority.name}</span></div>`
			}));
			return renderSnippet(statusCellSnippet);
		},
		enableGlobalFilter: false,
