...
class DatabaseSeeder {
	...
	private async fetchExistingData(): Promise<void> {
		...
		try {
			...
			if (this.existingStatuses.length === 0) {
				throw new Error(
					"No statuses found in database. Please run 'tsx seed-base-data.ts' first to create base data."
				);
			}
			if (this.existingPriorities.length === 0) {
				throw new Error(
					"No priorities found in database. Please run 'tsx seed-base-data.ts' first to create base data."
				);
			}
			if (this.existingCategories.length === 0) {
				throw new Error(
					"No categories found in database. Please run 'tsx seed-base-data.ts' first to create base data."
				);
			}

			this.log(
				`✓ Found ${this.existingStatuses.length} statuses, ${this.existingPriorities.length} priorities, ${this.existingCategories.length} categories`
			);
		} catch (error) {
		...
		for (let i = 0; i < this.options.requesterCount; i++) {
			...
			const [requester] = await db
				...
				.values({
					...
					email,
					phone: hasPhone
						? `+31 ${Math.floor(Math.random() * 90 + 10)} ${Math.floor(Math.random() * 9000000 + 1000000)}`
						: null
				})
				.returning();
	...
	private async createTickets(): Promise<void> {
		...
		const ticketChannels: ('email' | 'portal' | 'user' | 'dashboard')[] = [
			'email',
			'portal',
			'user',
			'dashboard'
		];
		const messageChannels: ('email' | 'portal' | 'system' | 'api' | 'dashboard')[] = [
			'email',
			'portal',
			'dashboard'
		];
		...
		for (let i = 0; i < this.options.ticketCount; i++) {
			...
			subject = subject.replace(
				'{service}',
				['Slack', 'GitHub', 'Salesforce'][Math.floor(Math.random() * 3)]
			);
	...
	private async createAttachments(
		...
		for (let i = 0; i < numAttachments; i++) {
			...
			const fileSize =
				Math.floor(Math.random() * (fileType.maxSize - fileType.minSize)) + fileType.minSize;
	...
	private async createTicketNotification(
		...
		for (const user of recipients) {
			...
			await db.insert(schema.userNotification).values({
				...
				readAt:
					Math.random() > 0.6
						? new Date(createdAt.getTime() + Math.random() * 2 * 60 * 60 * 1000)
						: null,
				sentViaEmail: emailSent,
				emailSentAt: emailSent
					? new Date(createdAt.getTime() + Math.random() * 5 * 60 * 1000)
					: null,
	...
	private async createSystemNotifications(): Promise<void> {
		...
		for (let i = 0; i < this.options.notificationCount; i++) {
			...
			message = message.replace(
				'{feature}',
				['Advanced Search', 'Bulk Actions', 'Custom Fields', 'Email Templates'][
			...
			const numRecipients =
				Math.floor(Math.random() * (this.createdUsers.length / 2)) +
				Math.ceil(this.createdUsers.length / 2);
				...
				await db.insert(schema.userNotification).values({
					...
					emailError:
						!emailSent && shouldSendEmail
							? ['Network error', 'Invalid email', 'Recipient blocked'][
	...
	private async createTasks(): Promise<void> {
		...
		for (let i = 0; i < totalTasks; i++) {
			...
			const ticket =
				Math.random() < 0.3 && this.createdTickets.length > 0
		...
		for (let i = 0; i < tasksWithSubtasks; i++) {
			...
			for (let j = 0; j < numSubtasks; j++) {
				...
				const createdAt = new Date(
					parentTask.createdAt.getTime() + subtaskAge * 24 * 60 * 60 * 1000
				);
