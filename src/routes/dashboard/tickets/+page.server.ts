...
export const load: PageServerLoad = async ({ url, depends }) => {
	...
	if (assigneeFilter) {
		if (assigneeFilter === 'unassigned') {
			...
			conditions.push(eq(schema.ticket.assigneeId, parseInt(assigneeFilter)));
		}
	}
	...
	const tickets = await db
		.select({
			...
			assigneeId: schema.ticket.assigneeId,
		...
		.leftJoin(schema.user, eq(schema.ticket.assigneeId, schema.user.id))
